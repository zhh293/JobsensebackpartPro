...
/*
    如果说用户想要查看关联的文件，比如头像，那么一般存入数据库的是文件所在地址，那么我拿到这个地址从本机去找的时候，返回给用户的是甚么，Resource吗
    是的，你说得对。在Spring Boot中，当你从数据库获取文件路径并需要返回给用户时，通常会使用Resource或其子类来处理文件传输。Resource接口提供了一种统一的方式来访问不同类型的资源（如文件系统中的文件、类路径中的资源、URL资源等）。*/
